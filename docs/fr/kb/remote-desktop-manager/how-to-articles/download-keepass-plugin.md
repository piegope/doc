...
1. En raison de certains changements effectués dans Remote Desktop Manager, le module d’extension de KeePass doit être téléchargé manuellement à l’aide du lien suivant : [module d’extension Keepass](https://cdn.devolutions.net/download/Devolutions.RemoteDesktopManager.KeePassPlugin.2.1.2.0.zip). 
...
3. Créer une entrée KeePass dans Remote Desktop Manager. Ensuite, vérifier que le module d’extension de KeePass est bel et bien installé. 
...
Par défaut, {{ fr.RDM }} va sélectionner la base de données. Cette option peut cependant être changée manuellement en saisissant un nouveau chemin. Remote Desktop Manager permet également de relier plusieurs bases de données KeePass simultanément. Ajouter tous les chemins en les séparant par un point-virgule. 
