...
{% snippet icon.badgeInfo %} 
{{ fr.DVLS }} valide que les niveaux fonctionnels du domaine et de la forêt ont le minimum requis Windows Server 2012R2. Cela est essentiel lors de l'utilisation de l'authentification d'Active Directory. 
{% endsnippet %}

Le tableau suivant énumère les spécifications matérielles minimales recommandées pour {{ fr.DVLS }}. Ces spécifications peuvent varier en fonction de l'utilisation de l'application, comme indiqué plus loin dans l'article.

|Implémentation de base{width=220px} |Implémentation moyenne{width=250px} |Implémentation large|
|:--------------------------: |:------------------------: |:---------------------: |
|(1-20 utilisateurs simultanés)<br>Avec Serveur SQL autonome<br>Processeurs à 4 coeurs<br>8 Go RAM<br>Adaptateur de réseau (1 Go)<br>|(21-75 utilisateurs simultanés)<br>Avec Serveur SQL autonome<br>Processeurs à 4 coeurs<br>8 Go RAM<br>Adaptateur de réseau (1 Go)<br>|(75+ utilisateurs simultanés) <br>Nous recommandons le déploiement de plusieurs {{ fr.DVLS }} dans une topologie d'équilibrage de charge avec un Serveur SQL à haute disponibilité.|
