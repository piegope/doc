...
1. Installer la ***Console de {{ fr.DVLS }}***  sur le serveur Web. Elle est téléchargeable sur la [page de téléchargement](https://server.devolutions.net/fr/home/download) . 
1. Exécuter la ***Console de {{ fr.DVLS }}*** avec des privilèges élevés (Exécuter en tant qu'administrateur). Cliquer avec le bouton droit sur l'icône de l'application, puis sélectionner Exécuter en tant qu'administrateur . 

{% snippet icon.shieldWarning %} 
Toutes les opérations effectuées via la ***Console de {{ fr.DVLS }}***  seront faites avec l'identifiant qui a lancé la ***Console de {{ fr.DVLS }}***  . Si vous devez utiliser un autre identifiant pour lancer la ***Console de {{ fr.DVLS }}***  , vous devez quitter la session Windows et vous reconnecter avec l'identifiant en question. La commande RunAs n'offre pas la possibilité de démarrer une application en tant qu'administrateur. La commande Exécuter en tant qu'autre utilisateur fonctionne seulement si le compte utilisé est membre du groupe d'administrateurs local de la machine. 
{% endsnippet %}
 
![Exécuter en tant qu'administrateur](https://webdevolutions.azureedge.net/docs/fr/server/ServerOp8139.png) 
 
3. Dans la ***Console de {{ fr.DVLS }}***  , cliquer sur le bouton ***Nouveau*** ou sur ***Installer une nouvelle instance*** pour déployer une nouvelle instance.  
...
12. Dans la ***Configuration du mot de passe de la console*** ***{{ fr.DVLS }}*** , lors de la configuration d'un mot de passe, l'instance {{ fr.DVLS }} sera protégée par un mot de passe qui sera enregistré dans la base de données.  
...
Pour tester l'installation du serveur, accédez à l'URL de l'instance (par exemple: http<area>://<Machine_Name>/<InstanceName>) avec n'importe quel navigateur Web ou cliquer sur le bouton ***Accéder au site Web*** dans la Console de {{ fr.DVLS }} .  
