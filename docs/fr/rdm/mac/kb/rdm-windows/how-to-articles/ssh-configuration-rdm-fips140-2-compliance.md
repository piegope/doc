---
eleventyComputed:
  title: Configuration SSH pour la conformité FIPS 140-2 de {{ fr.RDM }}
---
Pour que le client SSH de {{ fr.RDM }} soit conforme à l'annexe A de la norme FIPS 140-2, vous devez appliquer des modifications de configuration spécifiques dans les options de {{ fr.RDM }}.
...
Nous ne recommandons pas d'utiliser ces paramètres à moins que vous ne soyez dans un environnement qui exige la conformité avec le FIPS 140-2. La plupart des utilisateurs devraient s'en tenir aux paramètres par défaut de {{ fr.RDM }} et activer des suites de chiffrement supplémentaires uniquement si requis par leurs serveurs.
{% endsnippet %}
