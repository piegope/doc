...
1. Créer une entrée de Tunnel SSH en saisissant les informations du serveur SSH par lequel nous devons nous authentifier pour accéder à la zone DMZ (#1). Puis, dans les Paramètres de Tunnel Sortant, configurer le Mode sur Dynamique et la Source sur 127.0.0.1 (#2).
...
1. Configurer l'Hôte, le Port, le Nom d'utilisateur, le Domaine et le Mot de passe qui se réfèrent à votre session RDP (oublier l'ancienne configuration, ne pas saisir une IP localhost 127.0.0.1).
...
{type="a"}
    1. ***Groupe VPN*** peut être configuré comme expliqué dans [Configurer VPN pour plusieurs sessions](/rdm/mac/kb/rdm-windows/how-to-articles/configure-vpn-multiple-sessions/). Nous recommandons fortement de configurer le Groupe VPN si de nombreuses sessions RDP/Navigateur Web utilisent la même Passerelle Sécurisée.
