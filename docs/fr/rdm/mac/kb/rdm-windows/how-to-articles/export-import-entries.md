...
1. Dans la fenêtre ***Exporter les entrées***, cocher les options que vous souhaitez activer. Pour une migration typique, cocher les options ***Inclure les identifiants, Inclure les pièces jointes et documents***, et ***Chiffrer avec la clé principale***.
   ...
   Spécifier une clé principale chiffrera l'intégralité du contenu du fichier **.rdm** pour le protéger. C'est fortement recommandé comme mesure de sauvegarde, mais la clé est absolument nécessaire pour le déchiffrement. La stocker sur un dispositif de stockage séparé pour une sécurité accrue.
   {% endsnippet %}
