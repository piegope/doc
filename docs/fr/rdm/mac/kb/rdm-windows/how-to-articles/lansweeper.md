...
---
Pour les personnes utilisant actuellement Lansweeper ou envisageant son ajout à leur boîte à outils informatique, l'intégration dans {{ fr.RDM }} offre des avantages substantiels. Elle permet aux utilisateurs de visualiser de manière transparente tous les actifs Lansweeper dans {{ fr.RDM }}. De plus, cette intégration facilite la synchronisation automatique et la génération d'entrées pour les actifs Lansweeper, améliorant considérablement l'efficacité et l'efficacité des processus de gestion informatique.

## Lier Lansweeper aux propriétés d'une entrée via l'onglet de gestion des actifs informatiques
La fonctionnalité de Gestion des Actifs Informatiques peut être utilisée pour lier un gestionnaire d'actifs (par exemple, [BlueTally](/rdm/mac/kb/rdm-windows/how-to-articles/it-asset-management/), Lansweeper) à travers les propriétés d'une entrée. Un onglet intégré affichera alors la page principale de Lansweeper. Vous pouvez également basculer facilement entre vos données dans {{ fr.RDM }} et Lansweeper.
...
1. Entrer l'URL vers un actif spécifique.
1. Taper un ***titre de gestion des actifs informatiques*** dans le champ correspondant.
1. Cliquer sur ***Mettre à jour*** pour sauvegarder les changements et fermer la fenêtre.
...
* ***Modèle*** : Modèle Lansweeper créé dans {{ fr.RDM }}.
* ***Vérification des doublons*** ne va pas importer les entrées ayant le même type et hôte.
...
### Onglet Filtre
* ***Utiliser une requête personnalisée*** : Si nos paramètres pris en charge ne satisfont pas vos besoins, ou si vous avez déjà une requête pré-faite, vous pouvez l'entrer dans cette section. Suivant la documentation de Lansweeper [ici](https://docs.lansweeper.com/docs/api/getting-data#filtered-query), ce que vous devez entrer dans {{ fr.RDM }} est tout ce qui se trouve dans les crochets "filters:". Comme exemple, voici un filtre qui récupère les Systèmes d'Alarme et les appareils Android, avec leur nom contenant la valeur "asset".
...
* ***Ajouter une Condition*** :  Ajouter une condition au filtre. Vous devez d'abord entrer la valeur de la propriété, suivie de l'opérateur que vous souhaitez appliquer à cette condition.

![Onglet Filtre](https://cdnweb.devolutions.net/docs/docs_en_kb_KB6171.png)

### Onglet Avancé
* ***Préfixe du nom de session*** : Un préfixe pour l'entrée qui sera créée à travers le synchroniseur
* ***Suffixe du nom de session*** : Un suffixe pour l'entrée qui sera créée à travers le synchroniseur
* ***Action en cas de non-concordance d'entrée*** : L'action qui sera effectuée lorsque les entrées précédemment créées par le synchroniseur ne correspondent pas aux données trouvées lors de la synchronisation.
