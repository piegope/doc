---
eleventyComputed:
  title: Dépannage de l'installation de Microsoft Edge WebView2 Runtime
...
**L'installation a échoué. Le Microsoft Edge WebView2 Runtime est déjà installé pour le système.**
...
1. Sauvegarder le fichier de clé de registre dans un emplacement dont vous vous souviendrez, par exemple dans **C:\Program Files (x86)\Microsoft\EdgeWebView2**.
1. Faire un clic droit sur la clé dans l'Éditeur du Registre et sélectionner ***Supprimer***.
1. Installer la dernière version du bootstrapper. Consulter [Télécharger Microsoft Edge Chromium](/rdm/mac/kb/rdm-windows/knowledge-base/download-microsoft-edge-chromium/) pour vous assurer de télécharger la version compatible avec {{ fr.RDM }}.
1. Une fois téléchargé et installé, redémarrer {{ fr.RDM }} pour que l'installation prenne effet.

{% snippet, "badgeHelp" %}
Si vous rencontrez toujours des problèmes avec l'installation de Microsoft Edge WebView2 Runtime avec {{ fr.RDM }}, contactez notre équipe de support à [service@devolutions.net](mailto:service@devolutions.net).
{% endsnippet %}
