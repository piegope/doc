...
Le terme 'connect-viserver' n'est pas reconnu comme le nom d'une applet de commande, fonction, fichier de script ou programme exécutable. vérifiez l'orthographe du nom, ou si un chemin a été inclus, vérifiez que le chemin est correct et réessayez.
...
Si vous redémarrez {{ fr.RDM }} avec l'utilisateur actuel (sans exécuter en tant qu'admin) et ouvrez l'entrée du tableau de bord VMware, après 30 secondes, vous devriez voir tous vos serveurs.
...
Veuillez consulter votre administrateur pour vous assurer que cette commande est autorisée à être utilisée au sein de votre infrastructure.
