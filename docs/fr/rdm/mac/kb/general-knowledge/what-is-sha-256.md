...
eleventyComputed:
  ...
  description: La famille de l'algorithme de hachage sécurisé (SHA) comprend plusieurs fonctions de hachage cryptographique, le SHA-256 étant une variante largement utilisée.
---
La famille de l'algorithme de hachage sécurisé (SHA) comprend plusieurs fonctions de hachage cryptographique, le SHA-256 étant une variante largement utilisée. Les hachages cryptographiques agissent comme des signatures numériques pour les données, permettant de comparer de grands ensembles de données par leurs valeurs de hachage plutôt que par leur contenu intégral. Cette méthode est similaire à l'utilisation d'empreintes digitales pour l'identification—chaque ensemble de données a un hachage unique, et même une modification mineure des données change la valeur du hachage.
