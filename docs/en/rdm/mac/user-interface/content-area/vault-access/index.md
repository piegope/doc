---
eleventyComputed:
  title: '{{ en.VLT }} access in {{ en.RDMMAC }}'
...
---
In {{ en.RDMMAC }}, users can request access to a public {{ en.VLT }}. An administrator / {{ en.VLT }} owner then has to approve the request.
...
{{ en.VLT }} visibility is of great importance in {{ en.RDM }}, as access requests can only be performed for public {{ en.VLT }}s and not private ones.
...
1. Go to ***Administration – System Settings – {{ en.VLT }} Management – {{ en.VLT }}***.
...
1. To modify an existing {{ en.VLT }}, select it in the list, then click on ***Edit***. To create a new {{ en.VLT }}, click on ***Add {{ en.VLT }}***.
...
If your {{ en.VLT }} is public, see how a user can [request access to the {{ en.VLT }}](/rdm/mac/user-interface/content-area/vault-access/request-vault-access) and how you can [approve this request](/rdm/mac/user-interface/content-area/vault-access/approve-vault-access).
{% endsnippet %}
