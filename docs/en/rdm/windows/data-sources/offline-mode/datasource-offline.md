...
The &#32; [Data Source Information](File_MyDataSourceInformation) displays the size of the offline cache file along with the effective modes (disabled, read-only, or read/write). 
### Caching mode 
...
Allows to view and use entries only. The content of the data source cannot be modified. 
	...
	<tr>
		<td>
[Read/Write](DataSource_OfflineReadWrite) 
		</td>
		<td>
...
{% snippet icon.badgeNotice %} 
To find out what offline mode is in effect when you are logged in, see [My Data Source Information](File_MyDataSourceInformation) . 
{% endsnippet %}
