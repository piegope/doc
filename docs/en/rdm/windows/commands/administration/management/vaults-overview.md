...
To learn about the different vault types and ways to access them, visit [Vaults](/rdm/windows/commands/view/panels/vault/).
{% endsnippet %}
...
{% snippet icon.badgeHelp %} 
If you are using a {{ en.DVLS }} or {{ en.DHUBB }} data source, vault management is done in their respective web interfaces. For more information, consult either [{{ en.DVLS }} vaults](/server/web-interface/administration/security-management/vaults/) or [{{ en.DHUB }} vaults](/hub/web-interface/hub-overview/administration/management/vaults/). 
{% endsnippet %}
...
To use an existing vault template, select it from the ***Template*** drop-down list. Unlike other settings, this one cannot be configured later since it changes how the vault is created. Learn about [***Templates***](/rdm/windows/commands/file/templates/) before deciding. 
{% endsnippet %}
