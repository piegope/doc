...
Avec l'authentification Windows, vous devez définir l'identité du pool d'applications sur un compte du domaine. Nous vous recommandons de créer un compte dédié à cet effet. Reportez-vous à [Configure {{ fr.DVLS }} to use domain single sign-on (SSO) (EN)](/kb/devolutions-server/how-to-articles/configure-server-use-domain-sso/) pour les instructions qui doivent être exécutées après la création de l'instance {{ fr.DVLS }}.
