...
Cette opération suppose que vous avez [{{ fr.DVLSCONSOLE }} installé](https://devolutions.net/server/home/download) sur votre hôte {{ fr.DVLSCONSOLE }} cible. Vous pouvez migrer une source de données MSSQL avant d'[installer complètement](/server/getting-started/installation/) une instance de {{ fr.DVLS }}.

{% snippet, "badgeCaution" %}
Cette procédure opère sur la base de données existante, pas une copie. Pour revenir en arrière, vous devriez restaurer à partir d'une sauvegarde, donc il est recommandé de faire une sauvegarde complète.
{% endsnippet %}
...
### {{ fr.DVLS }} gratuit
{% snippet, "badgeInfo" %}
Une licence {{ fr.RDM }} Team est nécessaire pour utiliser {{ fr.DVLS }} gratuitement.
{% endsnippet %}

{{ fr.DVLS }} offre une licence gratuite pour jusqu'à 10 utilisateurs. Lors de la première connexion à l'interface web {{ fr.DVLS }}, vous êtes invité à entrer une licence. Cliquer sur le bouton [***Activer la Version Gratuite***](/server/getting-started/installation/create-server-instance/#basic-installation) pour utiliser la licence gratuite.
