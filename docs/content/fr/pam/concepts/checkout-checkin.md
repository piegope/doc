...
eleventyComputed:
  ...
  description: Dans le contexte des comptes privilégiés (CP), la réservation et la restitution font référence à réserver un CP pour l'usage exclusif d'un utilisateur pendant une période définie.
---
Dans le contexte des comptes privilégiés (CP), ***réserver*** et ***restituer*** font référence à réserver un CP pour l'usage exclusif d'un utilisateur pendant une période définie. Il est prévu qu'une rotation de mot de passe se produise lors de la restitution, libérant le verrou exclusif. Ce processus peut se produire :
