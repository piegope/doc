...
* Un courriel de l'expéditeur est requis et stocké avec le GUID comme partie du "chargement" stocké dans la base de données.

## Comment utiliser {{ fr.DSEND_STANDALONE }}
1. Aller sur le site [{{ fr.DSEND_STANDALONE }}](https://send.devolutions.com/).
1. Entrer le ***Courriel de l'expéditeur***, qui est requis et stocké avec le GUID comme partie du "chargement" stocké dans la base de données.
1. Entrer votre ***Message sécurisé*** à chiffrer.
1. Ajouter une ***Phrase secrète*** pour une sécurité supplémentaire (obligatoire si le destinataire ne possède pas de {{ fr.DA }}, optionnel sinon).
