---
eleventyComputed:
  title: Comparer les éditions
  keywords:
    - rôle
---
Comparer les fonctionnalités entre nos éditions Gratuite et Équipe de {{ fr.RDM }}.

## Gestion de connexion à distance

| Fonctionnalité                                                   | Description                                                                                                                                                    | Gratuit                 | Équipe                 |
|------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------|------------------------|
| Lancer automatiquement des connexions en un seul clic            | Lancer des connexions vers des sessions privilégiées, des serveurs distants, des machines virtuelles, des sites Web et des applications en un seul clic en utilisant l'injection des identifiants. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Intégrer avec différents types de connexion et des add-ons       | Intégrer avec les protocoles les plus populaires et les types de connexion à distance ainsi qu'avec plus de 60 add-ons.                                       | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Gestion intégrée de VPN et de VPN                                 | Gestion de connexion VPN intégrée avec Microsoft, Cisco, SonicWall, et IPsecVPN ; supporte également la gestion de route VPN.                                  | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Exporter des sessions et des entrées                              | Exporter facilement des informations dans un fichier XML.                                                                                                      | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Importer d'autres applications et de différentes sources         | Importer des sessions d'autres applications comme Dameware, KeePass, RDCMan, et plus ; importer des informations d'un fichier CSV ou directement d'Active Directory. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Support de macro et macro automatisée                             | Supporter des variables personnalisées (hôte, nom d'utilisateur, domaine, mot de passe) et des variables d'environnement Windows ; supporter également la macro de frappe automatique à la connexion ou à la déconnexion. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Macro automatisée                                                 | Macro de frappe automatique à la connexion ou à la déconnexion.                                                                                                 | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |

## Gestion de mot de passe

| Fonctionnalité                                                     | Description                                                                                                                                          | Gratuit                 | Équipe                 |
|---------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------|------------------------|
| Coffre-fort de mot de passe centralisé {{ fr.VLT }}                 | Protéger tous vos comptes privilégiés dans un coffre-fort centralisé {{ fr.VLT }} en utilisant un chiffrement approuvé par le gouvernement fédéral américain. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Stocker de manière sécurisée des informations sensibles             | Stocker toutes vos informations sensibles -- compte bancaire, email, passeport, carte de crédit, codes d'alarme -- dans un coffre-fort hautement sécurisé et centralisé {{ fr.VLT }}. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Générateur de mot de passe et outils de gestion de mot de passe intégrés | Générer des mots de passe forts et aléatoires, et avoir également un analyseur de mot de passe intégré et produire des rapports d'historique de mot de passe. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Héritage de mot de passe                                           | Définir votre mot de passe sur un dossier parent pour que les dossiers enfants l'héritent. Cela permet aux entrées d'hériter des identifiants d'un dossier parent. | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Gestionnaire de documents                                          | Stocker de manière sécurisée tous vos documents PDF, Word, Excel, Visio et images directement dans la base de données pour un partage facile.           | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Coffre-fort de l'utilisateur intégré {{ fr.UVLT }}                 | Permettre à chaque utilisateur d'avoir son propre coffre-fort {{ fr.UVLT }} auquel lui seul peut accéder.                                             | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
| Intégrer avec des gestionnaires de mot de passe existants          | Intégrer avec 1Password, AuthAnvil, Dashlane, KeePass, LastPass, et plus.                                                                            |                         | {{ icon.badgeNotice }} |
| Importer des mots de passe                                         | Importer directement des identifiants d'autres applications, y compris 1Password, KeePass, et LastPass.                                              |                         | {{ icon.badgeNotice }} |

## Accès sécurisé aux actifs critiques
...
| Support d'outil Macro/Script           | Supporter tout outil en ligne de commande, macros clavier, PowerShell, WASP PowerShell, et plus.                                                       | {{ icon.badgeNotice }} | {{ icon.badgeNotice }} |
