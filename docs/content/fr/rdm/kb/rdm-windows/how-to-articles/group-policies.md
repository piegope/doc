...
Pour en savoir plus sur la façon de déployer les modèles administratifs de {{ fr.RDM }} sur votre domaine, veuillez vous référer à la documentation de Microsoft.
...
#### Désactiver le remplissage automatique des identifiants de session de site Web et de site Web (obsolète) après une minute
...
#### Autoriser uniquement la création d'identifiants lorsqu'ils sont dans le {{ fr.UVLT }}
...
#### Désactiver tous les outils d'application locale comme le Visualiseur d'événements ou IIS
`%Root%\SOFTWARE\Policies\Devolutions\RemoteDesktopManager\DisableApplicationTools`

---

#### Masquer tous les panneaux d'enregistrements dans le tableau de bord
`%Root%\SOFTWARE\Policies\Devolutions\RemoteDesktopManager\DashboardHideAllRecordingsPanels`

---

#### Désactiver le glisser-déposer dans la liste de connexion
