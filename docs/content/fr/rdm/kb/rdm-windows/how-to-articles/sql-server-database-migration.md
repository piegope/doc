...
1. Sélectionner la ***Source*** de votre ***type de serveur*** et la ***Cible*** de votre ***type de serveur*** dans le menu déroulant et laisser le ***Périmètre de migration*** à ***Schéma et données***. Cliquer sur ***Créer***.
...
1. Entrer le ***Nom du serveur local*** et choisir un ***Type d'authentification***.
   {% snippet, "badgeInfo" %}
   Assurez-vous d'avoir suffisamment de droits et d'autorisations pour effectuer cette action.
   {% endsnippet %}
1. Cliquer sur ***Connecter***.
1. Choisir votre base de données dans la sélection chargée et cliquer sur ***Suivant***.
...
1. Choisir votre base de données dans la sélection chargée et cliquer sur ***Suivant***.
   ...
   Votre nouvelle base de données doit déjà avoir été [créée](https://docs.microsoft.com/en-us/azure/azure-sql/database/single-database-create-quickstart) pour apparaître dans cette liste.
   {% endsnippet %}
...
1. Valider si vous avez des problèmes en faisant défiler la liste des objets de schéma à gauche. Vous pouvez cliquer sur un élément pour plus de détails sur le problème spécifique et si une correction est disponible.
   ...
   Ces utilisateurs spécifiques devront [exporter](/rdm/kb/rdm-windows/how-to-articles/export-import-entries/) leur ***{{ fr.UVLT }}*** et leur configuration avant la migration. Ne pas le faire entraînera la perte des données enregistrées sous ces sections : ***Mes paramètres de compte***, ***Paramètres spécifiques à l'utilisateur***, et toute entrée faite dans leur ***{{ fr.UVLT }}***.
...
1. Lorsque tous les problèmes sont corrigés ou désélectionnés, cliquer sur ***Générer le script SQL***.
![!!KB4731](https://cdnweb.devolutions.net/docs/docs_en_kb_KB4731.png)
1. Une fois le script généré, valider s'il y a des problèmes, puis cliquer sur ***Déployer le schéma***.
...
1. Une fois les ***Résultats du déploiement*** terminés, valider s'il y a des problèmes, puis cliquer sur ***Migrer les données***.
![!!KB4734](https://cdnweb.devolutions.net/docs/docs_en_kb_KB4734.png)
1. Cliquer sur ***Commencer la migration des données***. Notez que le nombre de tables peut être différent selon votre version.
![!!KB4735](https://cdnweb.devolutions.net/docs/docs_en_kb_KB4735.png)
1. Attendre que la migration soit terminée. Lorsque c'est fait, vous pouvez fermer l'***Assistant de migration de données***.
...
1. Faire un clic droit sur le ***Nœud Bases de données – Importer l'application de niveau de données…***.
1. Suivre les étapes de l'assistant.
