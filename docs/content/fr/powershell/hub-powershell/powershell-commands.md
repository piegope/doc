...
eleventyComputed:
  ...
  description: Ce sujet concerne toutes les commandes d'authentification et les manipulations pour PSHubContext.
  keywords:
  - PowerShell
  - Authentification
---
...
#### Description
Si PSHubContext est fourni, tente de se connecter à {{ fr.DHUB }} avec.

ou  

Si PSHubContext n'est **pas** fourni, crée un PSHubContext avec les informations reçues et tente de se connecter à {{ fr.DHUB }} ensuite.
...
Si SecurePassword est fourni, demande à l'utilisateur d'entrer un mot de passe pour chiffrer le fichier .json.
...
Avec un argument SecurePassword qui demandera la saisie du mot de passe sécurisé.  
```powershell
Save-HubContext -Path “C:\PATH\TO\YOUR\FILE\file.json“ -SecurePassword
```
```powershell
Mot de passe : [INSÉREZ VOTRE MOT DE PASSE]
```
...
Si SecurePassword est fourni, demande à l'utilisateur d'entrer un mot de passe pour déchiffrer le fichier .json.
...
#### Description
Définit les paramètres système fournis sur {{ fr.DHUB }}.
...
#### Description
Définit le {{ fr.VLT }} fourni au VaultId mentionné.
