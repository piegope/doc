...
* If {{ en.RDM }} is slow to start on the jump host, especially if it is running on TS or similar environments. See [Startup Performance](/rdm/kb/rdm-windows/troubleshooting-articles/startup-performance/) for a few quick solutions to improve {{ en.RDM }} startup times. You can also uncheck the ***Show Splash screen*** setting in ***File – Settings – Application – Application Startup*** to disable the splash screen.
