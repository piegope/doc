...
| ***File***        | This field points to the SSH key file (this field is only available if the ***Entry type*** is ***File (local)***). |
| ***Private key*** | Select or generate a private key file (this field is only for ***Embedded data*** and the key is saved in {{ en.RDM }}). |
...
| ***Double-click delimiters***              | Delimits what is selected in the terminal with a character or a string when double-clicking. |
...
| ***Log mode***           | The ***Event*** mode only logs core events. The ***Printable output*** mode logs everything happening visibly in the terminal. |
...
| ***Macro list source***   | The ***Entire vault*** option only lists macros from the current vault. The ***Custom*** option selects only specific macros. |
...
| ***Ignore Substitute character***               | Ignores substitute characters or not.                            |
