...
{% snippet icon.badgeInfo %}
Most Connection Components requiring an application to be opened on PSM will require "Open Externally" enabled. This is due to a limitation of the RDP ActiveX we're using not supporting RDP RemoteApp mode.
It is however possible to mitigate this limitation by setting a parameter of the Connection Components in CyberArk.
The setting is DisableRemoteApp set to "yes". MOre on the matter on CyberArk Documentation Page https://docs.cyberark.com/PrivCloud-SS/Latest/en/Content/Privilege%20Cloud/PrivCloud-psm-UX.htm.
{% endsnippet %}  
